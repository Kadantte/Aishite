						...
						{this.props.items.map((item, index) => {
							return (
								<Button key={index} color={this.state.index === index ? Color.DARK_500 : undefined} height={40}
									...
									<Text left={15} children={item.first.split(this.props.highlight!).map((text, index, array) => array.length > index + 1 ? text.isEmpty() ? { text: this.props.highlight, color: Color.RGBA_000 } : [{ text: text }, { value: this.props.highlight, color: Color.RGBA_000 }] : { text: text }).flat().map((text) => ({ ...text, size: 13.5 })) as never}></Text>
